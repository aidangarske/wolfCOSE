 ...
 * reasons. COSE_Key labels must be integers; text labels are rejected even
 * though COSE header maps permit them. Keys containing key_ops are rejected
 * with WOLFCOSE_E_UNSUPPORTED because the
 * fixed-size key wrapper cannot retain arbitrary operation identifiers.
 *
