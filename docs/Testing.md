...
algorithm coverage, code coverage, and failure injection testing. CI requires
100% line coverage for every wolfCOSE source file. These thresholds are
validated on every push and PR to ensure coverage does not regress.
...
round trips, and malformed-token negatives. The vectors were
generated by TF-M's `iat-verifier` as documented by RFC 9783.

`eat-psa-ext-sign-test` reruns the full profile with `WOLFCOSE_ENABLE_EXT_SIGN`
to exercise the delegated (external) signing path for PSA/EAT tokens.
