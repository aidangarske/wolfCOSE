...
COSE — CBOR Object Signing and Encryption, [RFC 9052](https://www.rfc-editor.org/rfc/rfc9052) and [RFC 9053](https://www.rfc-editor.org/rfc/rfc9053) — is the JOSE equivalent for binary, constrained-device protocols.
...
COSE wraps cryptographic operations around a CBOR payload. The structures are intentionally close to their JOSE cousins — if you have written JWS, `COSE_Sign` will look familiar — but every byte is CBOR.
...
The `*0` variants are the simple case: one signer, or one recipient, or one MAC. The non-`0` variants support multiple actors — multiple signers on the same payload (for hybrid classical/PQC migrations or multi-party approvals), multiple recipients on the same encrypted message (for fleet broadcast), or multiple MAC tags (for multicast groups).
...
- **CWT.** [CBOR Web Tokens](https://www.rfc-editor.org/rfc/rfc8392) — JWT but in CBOR. Used in OAuth profiles for constrained environments.
...
- [RFC 9052](https://www.rfc-editor.org/rfc/rfc9052) — CBOR Object Signing and Encryption (COSE): Structures and Process
- [RFC 9053](https://www.rfc-editor.org/rfc/rfc9053) — CBOR Object Signing and Encryption (COSE): Initial Algorithms
- [RFC 8949](https://www.rfc-editor.org/rfc/rfc8949) — Concise Binary Object Representation (CBOR)
- [IANA COSE registry](https://www.iana.org/assignments/cose/cose.xhtml) — algorithm identifiers and parameter assignments
- [SUIT working group](https://datatracker.ietf.org/wg/suit/about/) — firmware update standards built on COSE
