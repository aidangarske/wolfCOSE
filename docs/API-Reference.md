...
labels, and `bufSz` must be exactly the encoded length. COSE_Key text labels
are rejected even though COSE header maps permit text labels. See
...
integer labels only, no duplicates, `kty` required, and no trailing bytes.
Label `-1` is `crv` for
...
COSE header parsers accept both forms and bytewise track text-label duplicates,
including between protected and
unprotected header buckets. Registered COSE parameters remain numeric: a text
label such as `"alg"` is an unknown extension, not an alias for numeric label
1. Unknown non-critical extensions are skipped; an unknown entry listed in
numeric `crit` is rejected. `wc_CoseKey_Decode()` and
`wc_CoseKey_PeekInfo()` always require integer labels.
