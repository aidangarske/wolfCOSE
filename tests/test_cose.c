...
static void test_cose_sign1_ext_sign(void)
{
    ...
    if (ret == 0) {
        ret = wc_CoseSign1_Sign_ex(&signKey, WOLFCOSE_ALG_ES256,
            kid, sizeof(kid) - 1,
            payload, sizeof(payload) - 1,
            NULL, 0, NULL, 0,
            scratch, sizeof(scratch),
            out, sizeof(out), &outLen, NULL, WOLFCOSE_SIGN1_UNTAGGED);
        TEST_ASSERT(ret == 0 && outLen > 0, "ext-sign delegated sign");
        TEST_ASSERT(outLen > 0u && out[0] == 0x84u,
                    "ext-sign untagged output");
    }
