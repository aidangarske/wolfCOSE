...
static void test_interop_countersign_legacy(void)
{
    ...
    if (ret == 0) {
        ...
        /* Verification prefers a V2 label over a legacy one, so attaching V2
         * beside label 7 or 9 would hide the legacy signature; it is refused
         * at the plan stage, before any signer could run. */
        counterSigner.algId = WOLFCOSE_ALG_EDDSA;
        counterSigner.key = &counterKey;
        counterSigner.kid = NULL;
        counterSigner.kidLen = 0u;
        badRet = wc_Cose_AddCounterSignature(&counterSigner,
            countersign_legacy_message, sizeof(countersign_legacy_message),
            NULL, 0u, NULL, 0u, scratch, sizeof(scratch),
            out, sizeof(out), &outLen, NULL);
        TEST_ASSERT(badRet == WOLFCOSE_E_COSE_BAD_HDR,
                    "refuse V2 countersignature beside legacy label 7");
        badRet = wc_Cose_AddCounterSignature0(&counterSigner0,
            countersign0_legacy_message,
            sizeof(countersign0_legacy_message),
            NULL, 0u, NULL, 0u, scratch, sizeof(scratch),
            out, sizeof(out), &outLen, NULL);
        TEST_ASSERT(badRet == WOLFCOSE_E_COSE_BAD_HDR,
                    "refuse V2 countersignature beside legacy label 9");
    }
#endif
