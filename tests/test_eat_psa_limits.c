    ...
    const WOLFCOSE_EAT_PSA_TOKEN* token)
{
    ...
    (void)XMEMSET(&zero, 0, sizeof(zero));
    return ((token != NULL) &&
            (XMEMCMP(token, &zero, sizeof(zero)) == 0)) ? 1 : 0;
}
    ...
    int overLimit)
{
    ...
    if (overLimit != 0) {
        /* Satisfy the generic CBOR container check with minimal pairs. The EAT
         * component ceiling is tested before any pair is parsed. */
        for (i = 0u; (ret == WOLFCOSE_SUCCESS) && (i < (2u * count)); i++) {
    ...
    size_t* outLen)
{
    ...
    if ((ret == WOLFCOSE_SUCCESS) &&
        ...
        for (i = 0u; (ret == WOLFCOSE_SUCCESS) &&
             (i < (2u * (WOLFCOSE_EAT_PSA_MAX_CLAIMS + 1u))); i++) {
            ret = wc_CBOR_EncodeUint(&ctx, 0u);
        }
    }
    if ((ret == WOLFCOSE_SUCCESS) &&
