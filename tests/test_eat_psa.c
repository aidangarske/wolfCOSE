...
#define TEST_EAT_PSA_UNKNOWN_UINT64        0x400u
...
#define TEST_EAT_PSA_COMPONENT_UNKNOWN_UINT64 0x40u
    ...
    const WOLFCOSE_EAT_PSA_COMPONENT* component, uint32_t flags)
{
    ...
    count += ((flags & TEST_EAT_PSA_COMPONENT_DUP_UNKNOWN_CLAIM) != 0u) ?
             2u : 0u;
    count += ((flags & TEST_EAT_PSA_COMPONENT_UNKNOWN_UINT64) != 0u) ?
             1u : 0u;
    ...
    if ((ret == WOLFCOSE_SUCCESS) &&
        ((flags & TEST_EAT_PSA_COMPONENT_UNKNOWN_UINT64) != 0u)) {
        ret = wc_CBOR_EncodeUint(ctx, UINT64_MAX);
    }
    if ((ret == WOLFCOSE_SUCCESS) &&
        ((flags & TEST_EAT_PSA_COMPONENT_UNKNOWN_UINT64) != 0u)) {
        ret = wc_CBOR_EncodeUint(ctx, 1u);
    ...
    uint8_t* out, size_t outSz, size_t* outLen)
{
    ...
    if ((claims == NULL) || (profile == NULL) || (out == NULL) ||
        ...
        count += ((omit & TEST_EAT_PSA_UNKNOWN_UINT64) != 0u) ? 1u : 0u;
    ...
    if ((ret == WOLFCOSE_SUCCESS) &&
        ((omit & TEST_EAT_PSA_UNKNOWN_UINT64) != 0u)) {
        ret = wc_CBOR_EncodeUint(&ctx, UINT64_MAX);
    }
    if ((ret == WOLFCOSE_SUCCESS) &&
        ((omit & TEST_EAT_PSA_UNKNOWN_UINT64) != 0u)) {
    ...
    uint8_t* scratch, size_t scratchSz, WOLFCOSE_EAT_PSA_TOKEN* token)
{
    ...
    if (ret == WOLFCOSE_SUCCESS) {
        ...
        (void)XMEMSET(token, 0xA5, sizeof(*token));
    ...
    WOLFCOSE_EAT_PSA_COMPONENT* component)
{
    ...
    (void)XMEMSET(claims, 0, sizeof(*claims));
    ...
    WOLFCOSE_KEY* key)
{
    ...
    if ((resolver == NULL) || (key == NULL) ||
        ...
        (XMEMCMP(ueid, kUeid, sizeof(kUeid)) != 0) ||
    ...
    if ((componentCtx == NULL) || (component == NULL) ||
        ...
        ((componentCtx->expectedVersion != NULL) &&
         ((component->version.len != componentCtx->expectedVersionLen) ||
          ...
          (XMEMCMP(component->version.data, componentCtx->expectedVersion,
    ...
    const WOLFCOSE_EAT_PSA_COMPONENT* component)
{
    if ((ctx != NULL) || (component == NULL) ||
        ...
        (component->measurementValue.len != sizeof(kMeasurement)) ||
        (XMEMCMP(component->measurementValue.data, kMeasurement,
...
static void test_eat_psa_nonce_and_hash_lengths(WOLFCOSE_KEY* key, WC_RNG* rng)
{
    ...
    for (i = 0u; i < (sizeof(lengths) / sizeof(lengths[0])); i++) {
        ...
        if (ret == WOLFCOSE_SUCCESS) {
            (void)XMEMSET(&componentCtx, 0, sizeof(componentCtx));
        ...
        if (tokenLen > 0u) {
            (void)XMEMCPY(wrongNonce, nonce, lengths[i]);
            wrongNonce[0] ^= 0xFFu;
...
static int test_eat_psa_token_is_zero(const WOLFCOSE_EAT_PSA_TOKEN* token)
{
    ...
    (void)XMEMSET(&zeroToken, 0, sizeof(zeroToken));
    return ((token != NULL) &&
            (XMEMCMP(token, &zeroToken, sizeof(zeroToken)) == 0)) ? 1 : 0;
}
    ...
    uint8_t* out, size_t outSz, size_t* outLen)
{
    ...
    if ((in == NULL) || (find == NULL) || (replacement == NULL) ||
        ...
        for (i = 0u; i <= (inLen - findLen); i++) {
            if (XMEMCMP(&in[i], find, findLen) == 0) {
                (void)XMEMCPY(out, in, i);
                (void)XMEMCPY(&out[i], replacement, replacementLen);
                (void)XMEMCPY(&out[i + replacementLen], &in[i + findLen],
                    inLen - i - findLen);
    ...
    const uint8_t* in, size_t inSz, uint8_t* scratch, size_t scratchSz)
{
    ...
    (void)XMEMCPY(scratch, kNonce, sizeof(kNonce));
    ret = wc_CoseEatPsaToken_Verify(key, in, inSz, scratch, sizeof(kNonce),
        scratch, scratchSz, &token);
    TEST_ASSERT(ret == WOLFCOSE_SUCCESS &&
                token.nonce.len == sizeof(kNonce) &&
                XMEMCMP(token.nonce.data, kNonce, sizeof(kNonce)) == 0,
        "preserve expected nonce that aliases verification scratch");

    (void)XMEMCPY(&token, kNonce, sizeof(kNonce));
    ret = wc_CoseEatPsaToken_Verify(key, in, inSz,
        (const uint8_t*)&token, sizeof(kNonce), scratch, scratchSz, &token);
    TEST_ASSERT(ret == WOLFCOSE_SUCCESS &&
                token.nonce.len == sizeof(kNonce) &&
                XMEMCMP(token.nonce.data, kNonce, sizeof(kNonce)) == 0,
        "preserve expected nonce that aliases verification output");
    ...
    (void)XMEMCPY(scratch, kNonce, sizeof(kNonce));
    ret = wc_CoseEatPsaToken_VerifyByUeid(test_eat_psa_resolve, &resolver,
        in, inSz, scratch, sizeof(kNonce), scratch, scratchSz, &token);
    TEST_ASSERT(ret == WOLFCOSE_SUCCESS && resolver.called == 1 &&
                token.nonce.len == sizeof(kNonce) &&
                XMEMCMP(token.nonce.data, kNonce, sizeof(kNonce)) == 0,
        "preserve UEID-resolver nonce that aliases verification scratch");

    resolver.called = 0;
    (void)XMEMCPY(&token, kNonce, sizeof(kNonce));
    ret = wc_CoseEatPsaToken_VerifyByUeid(test_eat_psa_resolve, &resolver,
        in, inSz, (const uint8_t*)&token, sizeof(kNonce), scratch, scratchSz,
        &token);
    TEST_ASSERT(ret == WOLFCOSE_SUCCESS && resolver.called == 1 &&
                token.nonce.len == sizeof(kNonce) &&
                XMEMCMP(token.nonce.data, kNonce, sizeof(kNonce)) == 0,
        "preserve UEID-resolver nonce that aliases verification output");
    ...
    WC_RNG* rng)
{
    ...
    if ((ret == WOLFCOSE_SUCCESS) &&
        ((messageLen < sizeof(expectedPrefix)) ||
         (XMEMCMP(message, expectedPrefix, sizeof(expectedPrefix)) != 0))) {
        TEST_ASSERT(0, "expected deterministic Sign1 header layout");
        ret = WOLFCOSE_E_CBOR_MALFORMED;
    }
    if (ret == WOLFCOSE_SUCCESS) {
        (void)XMEMCPY(nonPreferred, message, sizeof(expectedPrefix) - 1u);
        nonPreferred[sizeof(expectedPrefix) - 1u] = 0xB8u;
        nonPreferred[sizeof(expectedPrefix)] = 0x00u;
        (void)XMEMCPY(&nonPreferred[sizeof(expectedPrefix) + 1u],
        ...
        TEST_ASSERT(testRet == WOLFCOSE_SUCCESS &&
                    ...
                    verifiedPayloadLen == sizeof(kNonce) &&
                    XMEMCMP(verifiedPayload, kNonce, sizeof(kNonce)) == 0,
    ...
    uint8_t* scratch, size_t scratchSz)
{
    ...
    (void)XMEMCPY(badUeid, kUeid, sizeof(badUeid));
    badUeid[0] = 0x02u;
    caseClaims = *claims;
    ...
    ret = test_eat_psa_verify_raw_current_ex(key, rng, claims,
        currentProfile, sizeof(currentProfile) - 1u,
        TEST_EAT_PSA_UNKNOWN_UINT64, 0u, 0,
        TEST_EAT_PSA_COMPONENT_UNKNOWN_UINT64, claims->componentCount,
        scratch, scratchSz, &token);
    TEST_ASSERT(ret == WOLFCOSE_SUCCESS && token.componentCount == 1u,
        "accept full-width unknown integer claims and components");
    ...
    if (testRet == WOLFCOSE_SUCCESS) {
        ...
        for (i = 0u; (i + (sizeof(currentProfile) - 1u)) <=
            ...
            if (XMEMCMP(&invalidUtf8Payload[i], currentProfile,
    ...
    uint8_t* storage, size_t storageSz)
{
    ...
    if ((span != NULL) && (span->data != NULL) && (span->len > 1u) &&
        (span->len <= storageSz)) {
        (void)XMEMCPY(storage, span->data, span->len);
        span->data = storage;
        ret = 1;
    }
...
static void test_eat_psa_issue_boundaries(WOLFCOSE_KEY* key, WC_RNG* rng)
{
    ...
    test_eat_psa_claims(&claims, &component);
    (void)XMEMSET(out, 0xA5, sizeof(out));
    outLen = 17u;
...
static void test_eat_psa_rfc9783_sign1(void)
{
    ...
    if (ret == WOLFCOSE_SUCCESS) {
        ...
        (void)XMEMCPY(&nonPreferred[2], &tokenBuf[1], tokenLen - 1u);
    ...
    if (ret == WOLFCOSE_SUCCESS) {
        cwtTagged[0] = 0xD8u;
        cwtTagged[1] = 0x3Du;
        (void)XMEMCPY(&cwtTagged[2], tokenBuf, tokenLen);
...
static void test_eat_psa_sign1(void)
{
    ...
    if (ret == WOLFCOSE_SUCCESS) {
        ...
        if ((tokenLen > 7u) && (tokenBuf[0] == 0xD2u) &&
            ...
            (void)XMEMCPY(unprotectedAlgToken, tokenBuf, 6u);
            (void)XMEMCPY(&unprotectedAlgToken[6], textUnprotected,
                sizeof(textUnprotected));
            (void)XMEMCPY(&unprotectedAlgToken[6u + sizeof(textUnprotected)],
            ...
            (void)XMEMCPY(unprotectedAlgToken, tokenBuf, 6u);
            (void)XMEMCPY(&unprotectedAlgToken[6], contentTypeUnprotected,
                sizeof(contentTypeUnprotected));
            (void)XMEMCPY(&unprotectedAlgToken[6u +
            ...
            (void)XMEMCPY(unprotectedAlgToken, tokenBuf, 6u);
            (void)XMEMCPY(&unprotectedAlgToken[6], nonPreferredTextUnprotected,
            ...
            (void)XMEMCPY(unprotectedAlgToken, tokenBuf, 6u);
            (void)XMEMCPY(&unprotectedAlgToken[6], duplicateTextUnprotected,
    ...
    if (ret == WOLFCOSE_SUCCESS) {
        ...
        if (tokenLen < sizeof(unprotectedAlgToken)) {
            (void)XMEMCPY(unprotectedAlgToken, tokenBuf, tokenLen);
    ...
    if (ret == WOLFCOSE_SUCCESS) {
        ...
        if (testRet == WOLFCOSE_SUCCESS) {
            ...
            if (token.components.len < sizeof(variantPayload)) {
                (void)XMEMCPY(variantPayload, token.components.data,
                    token.components.len);
    ...
    if (ret == WOLFCOSE_SUCCESS) {
        ...
        (void)XMEMCPY(malformedUeid, kUeid, sizeof(malformedUeid));
    ...
    if (ret == WOLFCOSE_SUCCESS) {
        if ((tokenLen > 7u) && (tokenBuf[0] == 0xD2u) &&
            (tokenBuf[1] == 0x84u) && (tokenBuf[6] == 0xA0u)) {
            (void)XMEMCPY(legacyToken, tokenBuf, 6u);
            legacyToken[6] = 0xA1u;
            legacyToken[7] = 0x18u;
            legacyToken[8] = 0x21u;
            legacyToken[9] = 0x40u;
            (void)XMEMCPY(&legacyToken[10], &tokenBuf[7], tokenLen - 7u);
    ...
    if (ret == WOLFCOSE_SUCCESS) {
        (void)XMEMCPY(legacyToken, tokenBuf, tokenLen);
    ...
    if (ret == WOLFCOSE_SUCCESS) {
        ...
        (void)XMEMCPY(wrongNonce, kNonce, sizeof(wrongNonce));
        wrongNonce[0] ^= 0xFFu;
        (void)XMEMSET(&token, 0xA5, sizeof(token));
    ...
    if (ret == WOLFCOSE_SUCCESS) {
        (void)XMEMCPY(lifecyclePayload, payload, payloadLen);
    ...
    if (ret == WOLFCOSE_SUCCESS) {
        (void)XMEMCPY(variantPayload, payload, 1u);
        variantPayload[1] = 0x18u;
        variantPayload[2] = 0x0Au;
        (void)XMEMCPY(&variantPayload[3], &payload[2], payloadLen - 2u);
    ...
    if (ret == WOLFCOSE_SUCCESS) {
        if ((payload[0] < 0xa0u) || (payload[0] >= 0xb7u)) {
            ...
            (void)XMEMCPY(&duplicatePayload[1], &payload[1], payloadLen - 1u);
            (void)XMEMCPY(&duplicatePayload[payloadLen], &payload[1], 35u);
    ...
    if (ret == WOLFCOSE_SUCCESS) {
        ...
        TEST_ASSERT(ret == WOLFCOSE_SUCCESS &&
                    ...
                    token.verificationServiceIndicator.len ==
                        claims.verificationServiceIndicator.len &&
                    XMEMCMP(token.verificationServiceIndicator.data,
                        claims.verificationServiceIndicator.data,
...
static void test_eat_psa_delegated_signer(void)
{
    ...
    test_eat_psa_claims(&claims, &component);
    (void)XMEMSET(&signCtx, 0, sizeof(signCtx));
    ret = wc_InitRng(&rng);
    if (ret == WOLFCOSE_SUCCESS) {
        rngInited = 1;
        ret = wc_ecc_init(&ecc);
    ...
    if (ret == WOLFCOSE_SUCCESS) {
        signCtx.fail = 1;
        (void)XMEMSET(tokenBuf, 0xA5, sizeof(tokenBuf));
        tokenLen = 17u;
        ret = wc_EatPsaToken_CreateSign1(&signKey, WOLFCOSE_ALG_ES256,
            &claims, claimsBuf, sizeof(claimsBuf), scratch, sizeof(scratch),
            tokenBuf, sizeof(tokenBuf), &tokenLen, NULL);
...
static void test_eat_psa_mac0(void)
{
    ...
    if (ret == WOLFCOSE_SUCCESS) {
        ...
        if ((tokenLen > 6u) && (tokenBuf[0] == 0xD1u) &&
            ...
            (tokenLen + sizeof(nonPreferredTextUnprotected) - 1u <=
             sizeof(modifiedToken))) {
            (void)XMEMCPY(modifiedToken, tokenBuf, 6u);
            (void)XMEMCPY(&modifiedToken[6], textUnprotected,
                sizeof(textUnprotected));
            (void)XMEMCPY(&modifiedToken[6u + sizeof(textUnprotected)],
                &tokenBuf[7], tokenLen - 7u);
            ...
            (void)XMEMCPY(modifiedToken, tokenBuf, 6u);
            (void)XMEMCPY(&modifiedToken[6], contentTypeUnprotected,
                sizeof(contentTypeUnprotected));
            (void)XMEMCPY(&modifiedToken[6u + sizeof(contentTypeUnprotected)],
                &tokenBuf[7], tokenLen - 7u);
            ...
            (void)XMEMCPY(modifiedToken, tokenBuf, 6u);
            (void)XMEMCPY(&modifiedToken[6], nonPreferredTextUnprotected,
                sizeof(nonPreferredTextUnprotected));
            (void)XMEMCPY(&modifiedToken[6u +
                sizeof(nonPreferredTextUnprotected)], &tokenBuf[7],
                tokenLen - 7u);
    ...
    if (ret == WOLFCOSE_SUCCESS) {
        ...
        (void)XMEMCPY(modifiedToken, tokenBuf, tokenLen);
    ...
    (void)XMEMSET(modifiedToken, 0xA5, sizeof(modifiedToken));
    outLen = 17u;
