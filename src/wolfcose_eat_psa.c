...
typedef struct WOLFCOSE_EAT_PSA_LABEL {
    uint64_t       val;
    const uint8_t* text;
    size_t         textLen;
    uint8_t        majorType;
} WOLFCOSE_EAT_PSA_LABEL;

static int wolfCose_EatPsaDecodeLabel(WOLFCOSE_CBOR_CTX* ctx,
    WOLFCOSE_EAT_PSA_LABEL* label)
{
    int ret;
    ...
    if ((ctx == NULL) || (label == NULL)) {
        ...
        ret = wolfCose_CBOR_DecodeHead_ex(ctx, &item,
            WOLFCOSE_EAT_PSA_DECODE_FLAGS);
    }
    if (ret == WOLFCOSE_SUCCESS) {
        (void)XMEMSET(label, 0, sizeof(*label));
        label->majorType = item.majorType;
        if ((item.majorType == WOLFCOSE_CBOR_UINT) ||
            (item.majorType == WOLFCOSE_CBOR_NEGINT)) {
            label->val = item.val;
        }
...
static int wolfCose_EatPsaLabelToInt(const WOLFCOSE_EAT_PSA_LABEL* label,
    int64_t* val)
{
    int ret = 0;

    if ((label != NULL) && (val != NULL) &&
        (label->val <= (uint64_t)INT64_MAX)) {
        if (label->majorType == WOLFCOSE_CBOR_UINT) {
            *val = (int64_t)label->val;
            ret = 1;
        }
        else if (label->majorType == WOLFCOSE_CBOR_NEGINT) {
            *val = -1 - (int64_t)label->val;
            ret = 1;
        }
        else {
            /* The label is a text string. */
        }
    }
...
static int wolfCose_EatPsaLabelsEqual(const WOLFCOSE_EAT_PSA_LABEL* first,
    const WOLFCOSE_EAT_PSA_LABEL* second)
{
    int ret = 0;

    if ((first != NULL) && (second != NULL) &&
        (first->majorType == second->majorType)) {
        if (first->majorType == WOLFCOSE_CBOR_TSTR) {
            if ((first->textLen == second->textLen) &&
                ((first->textLen == 0u) ||
                 (wolfCose_EatPsaConstantCompare(first->text, second->text,
                     first->textLen) == 0))) {
                ret = 1;
            }
        }
...
static int wolfCose_EatPsaMapHasPriorLabel(const uint8_t* map, size_t mapSz,
    size_t before, const WOLFCOSE_EAT_PSA_LABEL* label, int* duplicate)
{
    int ret;
    ...
    for (i = 0u; (ret == WOLFCOSE_SUCCESS) && (i < count) &&
         (scan.idx < before); i++) {
        WOLFCOSE_EAT_PSA_LABEL prior;

        ret = wolfCose_EatPsaDecodeLabel(&scan, &prior);
        if ((ret == WOLFCOSE_SUCCESS) &&
    ...
    WOLFCOSE_EAT_PSA_COMPONENT* component)
{
    ...
    for (i = 0u; (ret == WOLFCOSE_SUCCESS) && (i < count); i++) {
        ...
        ret = wolfCose_EatPsaDecodeLabel(ctx, &label);
        if (ret == WOLFCOSE_SUCCESS) {
            ret = wolfCose_EatPsaMapHasPriorLabel(&ctx->cbuf[mapStart],
                ctx->bufSz - mapStart, labelStart, &label, &duplicate);
            labelIsInt = wolfCose_EatPsaLabelToInt(&label, &labelVal);
        }
        if ((ret == WOLFCOSE_SUCCESS) && (duplicate != 0)) {
            ret = WOLFCOSE_E_EAT_PSA_CLAIM;
        }
        if ((ret == WOLFCOSE_SUCCESS) && (labelIsInt == 0)) {
            ret = WOLFCOSE_EAT_PSA_SKIP(ctx);
        }
        else if ((ret == WOLFCOSE_SUCCESS) &&
            (labelVal == WOLFCOSE_EAT_PSA_COMPONENT_TYPE)) {
            if ((seen & 0x01u) != 0u) {
                ret = WOLFCOSE_E_EAT_PSA_CLAIM;
            }
            else {
            ...
            (labelVal == WOLFCOSE_EAT_PSA_COMPONENT_VALUE)) {
            if ((seen & 0x02u) != 0u) {
                ret = WOLFCOSE_E_EAT_PSA_CLAIM;
            }
            else {
            ...
            (labelVal == WOLFCOSE_EAT_PSA_COMPONENT_VER)) {
            if ((seen & 0x04u) != 0u) {
                ret = WOLFCOSE_E_EAT_PSA_CLAIM;
            }
            else {
            ...
            (labelVal == WOLFCOSE_EAT_PSA_COMPONENT_SIGNER)) {
            if ((seen & 0x08u) != 0u) {
                ret = WOLFCOSE_E_EAT_PSA_CLAIM;
            }
            else {
            ...
            (labelVal == WOLFCOSE_EAT_PSA_COMPONENT_DESC)) {
            if ((seen & 0x10u) != 0u) {
                ret = WOLFCOSE_E_EAT_PSA_CLAIM;
            }
            else {
    ...
    size_t payloadLen, WOLFCOSE_EAT_PSA_TOKEN* token)
{
    ...
    for (i = 0u; (ret == WOLFCOSE_SUCCESS) && (i < count); i++) {
        ...
        if ((ret == WOLFCOSE_SUCCESS) && (labelIsInt == 0)) {
            ...
            wolfCose_EatPsaClaimType(labelVal, &bit, &type, &legacy);
            if (type == 0) {
                ret = WOLFCOSE_EAT_PSA_SKIP(&ctx);
            }
            else {
    ...
    WOLFCOSE_EAT_PSA_TOKEN* token)
{
    ...
    WOLFCOSE_HDR hdr = { 0 };
    uint8_t expectedNonceCopy[64] = { 0 };
    ...
    if ((key == NULL) || (in == NULL) || (expectedNonce == NULL) ||
        ...
        (void)XMEMCPY(expectedNonceCopy, expectedNonce, expectedNonceLen);
        (void)XMEMSET(token, 0, sizeof(*token));
    ...
    if (ret == WOLFCOSE_SUCCESS) {
        ...
        ret = wolfCose_EatPsaCheckNonce(token, expectedNonceCopy,
            expectedNonceLen);
    }
    if ((ret != WOLFCOSE_SUCCESS) && (token != NULL)) {
        (void)XMEMSET(token, 0, sizeof(*token));
    }
    wolfCose_ForceZero(expectedNonceCopy, sizeof(expectedNonceCopy));
    ...
    WOLFCOSE_EAT_PSA_TOKEN* token)
{
    ...
    if (ret == WOLFCOSE_SUCCESS) {
        ret = wc_CoseEatPsaToken_Verify(&key, in, inSz, expectedNonceCopy,
