...
TEST_SRC  = tests/test_cbor.c tests/test_cose.c tests/test_interop.c \
            ...
            tests/test_eat_psa.c tests/test_eat_psa_profiles.c \
            tests/test_main.c
...
rsapss-policy-test:
	$(CC) $(CFLAGS) $(WOLFCOSE_POLICY_DISABLE_HPKE_FLAGS) \
	    -Werror=unused-function -fsyntax-only \
	    -DWOLFCOSE_NO_SIGN1 -DWOLFCOSE_NO_SIGN $(SRC)
	$(CC) $(CFLAGS) $(WOLFCOSE_POLICY_DISABLE_HPKE_FLAGS) \
	    -x c -fsyntax-only -DWOLFSSL_NO_OPTIONS_H -DWC_NO_HARDEN \
	...
	trap 'rm -f "$$log_file"' 0 1 2 3 15; \
	if $(CC) $(CFLAGS) $(WOLFCOSE_POLICY_DISABLE_HPKE_FLAGS) -x c -fsyntax-only -DWOLFSSL_NO_OPTIONS_H -DWC_NO_HARDEN \
	...
	grep -q "RSA-PSS key validation requires WOLFSSL_EXPORT_INT" \
	    "$$log_file"
	$(CC) $(CFLAGS) $(WOLFCOSE_POLICY_DISABLE_HPKE_FLAGS) -x c -fsyntax-only -DWOLFSSL_NO_OPTIONS_H -DWC_NO_HARDEN \
	    -UHAVE_ECC -UWOLFSSL_EXPORT_INT \
	    -DWC_RSA_PSS -DWOLFCOSE_ENABLE_RSAPSS \
...
eat-psa-coverage: clean
	@set -e; for f in $(SRC); do \
	    $(CC) $(CFLAGS) $(EAT_PSA_FULL_FLAGS) --coverage -fprofile-arcs -ftest-coverage -c $$f -o $${f%.c}.o; \
	done
	rm -f $(LIB_A)
	$(AR) rcs $(LIB_A) $(OBJ)
...
eat-psa-coverage-force-failure: clean
	@set -e; for f in $(SRC); do \
	    $(CC) $(CFLAGS) $(EAT_PSA_FULL_FLAGS) -DWOLFCOSE_FORCE_FAILURE --coverage -fprofile-arcs -ftest-coverage -c $$f -o $${f%.c}.o; \
	done
	rm -f $(LIB_A)
	$(AR) rcs $(LIB_A) $(OBJ)
...
psa-eat-lean-verify:
	$(CC) $(CFLAGS) -DWOLFCOSE_LEAN_VERIFY \
		...
		-o $(EAT_LEANV_DEMO) \
		$(EAT_LEANV_DEMO).c $(SRC) $(LDFLAGS) $(LDLIBS)
