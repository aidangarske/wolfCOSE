...
/* Countersigning grows a message by a signature, a kid, and CBOR framing; one
 * capacity for input and output lets a countersigned file be countersigned or
 * verified again by the next command. */
#define WOLFCOSE_TOOL_MAX_COUNTER_MSG \
    (WOLFCOSE_TOOL_MAX_MSG + WOLFCOSE_TOOL_MAX_KEY + WOLFCOSE_MAX_SIG_SZ + 128u)
                      ...
                      size_t* outLen)
{
    ...
    /* A full buffer with bytes still unread would otherwise be processed as a
     * silently truncated message and fail later as malformed CBOR. */
    if (n == bufSz) {
        int next = fgetc(f);
    ...
    const char* detachedPath, const char* aadPath)
{
    ...
    int32_t keyAlg = 0;
    uint8_t keyBuf[WOLFCOSE_TOOL_MAX_KEY];
    uint8_t msgBuf[WOLFCOSE_TOOL_MAX_COUNTER_MSG];
    uint8_t detachedBuf[WOLFCOSE_TOOL_MAX_MSG];
    uint8_t aadBuf[WOLFCOSE_TOOL_MAX_MSG];
    uint8_t outBuf[WOLFCOSE_TOOL_MAX_COUNTER_MSG];
    uint8_t scratch[WOLFCOSE_MAX_SCRATCH_SZ];
    size_t keyLen = 0u;
    size_t msgLen = 0u;
...
int main(int argc, char* argv[])
{
    ...
    if (((detachedPath != NULL) || (aadPath != NULL)) &&
        (strcmp(cmd, "countersign") != 0) &&
        (strcmp(cmd, "counterverify") != 0)) {
        fprintf(stderr,
                "-p and --aad apply only to countersign and counterverify\n");
        return EXIT_USAGE;
    }
    if ((counterIndexSet != 0) && (strcmp(cmd, "counterverify") != 0)) {
